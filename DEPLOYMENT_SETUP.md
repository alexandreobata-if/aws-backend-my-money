...
Nome: `development-backend-dev-secret-rds` (ajuste conforme seu ambiente)
...
aws secretsmanager get-secret-value \
  --secret-id development-backend-dev-secret-rds \
  --region us-east-1 \
  --query 'SecretString' \
...
aws secretsmanager create-secret \
  --name development-backend-dev-secret-rds \
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:secretsmanager:us-east-1:YOUR_ACCOUNT_ID:secret:development-backend-dev-secret-rds-*"
    }
  ]
}
```
...
aws iam put-role-policy \
  ...
  --policy-document '{
    ...
    "Statement": [{
      "Effect": "Allow",
      "Action": ["secretsmanager:GetSecretValue"],
      "Resource": "arn:aws:secretsmanager:us-east-1:*:secret:development-backend-dev-secret-rds-*"
    }]
  }'
```
...
```bash
# Desenvolvimento
SECRET_NAME=development-backend-dev-secret-rds
